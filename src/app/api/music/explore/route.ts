...
export async function GET(req: NextRequest) {
  ...
  try {
    // Fetch up to 50 results to allow for randomization
    const res = await fetch(
      `https://api.deezer.com/search?q=${encodeURIComponent(q.trim())}&limit=50`,
      { next: { revalidate: 3600 } }
    );
    const data = await res.json();

    let deezerTracks = (data.data || []) as DeezerTrack[];
    
    // Shuffle the results to get random tracks for the mood/category
    deezerTracks.sort(() => 0.5 - Math.random());
    deezerTracks = deezerTracks.slice(0, limit);
    ...
    console.error("[Deezer Explore]", error);
    return NextResponse.json(
      { error: "Failed to explore music" },
      { status: 500 }
    );
  }
}
