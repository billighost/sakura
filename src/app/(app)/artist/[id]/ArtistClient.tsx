...
export function ArtistLoadingState() {
  return (
    <div style={{ padding: "0 clamp(1.25rem, 5vw, 2rem)" }}>
      <div style={{ padding: "clamp(3.5rem, 12vw, 4.5rem) 0 clamp(1.75rem, 5vw, 2.25rem)", textAlign: "center" }}>
...
export default function ArtistClient({ id }: { id: string }) {
    ...
    if (typeof window !== "undefined") {
      try {
        ...
        if (followedStr) {
          ...
          if (Array.isArray(followed) && followed.includes(id)) {
            setIsFollowing(true);
          }
        }
      } catch {}
    ...
    if (typeof window !== "undefined") {
      try {
        ...
        if (nextState) {
          ...
          followed = followed.filter((artistId) => artistId !== id);
        }
  ...
  return (
    <div className={styles.page}>
      <button className={styles.backBtn} onClick={() => back("/search")} aria-label="Go back">
